...
def generate_text(config, config_map, TotalData, question_info_map):
    ...
    for question in tqdm(config_map['question_map']):
        content = question_info_map[question]['content']
        tag = question_info_map[question]['concepts']
    ...
    for student in tqdm(range(len(config_map['stu_map']))):
        ...
        for log in student_logs.values:
            question = question_info_map[str(config_map['reverse_question_map'][log[1]])]['content']
            Name = question_info_map[str(config_map['reverse_question_map'][log[1]])]['concepts']
            if log[2] == 1:
...
def generate_embeddings_openai(config):
    ...
    with open('../data/MOOCRadar/embedding_openai.pkl', 'wb') as f:
        pickle.dump(config, f)    
    with open('MOOCRadar/result/embedding/embedding_openai.pkl', 'wb') as f:
        pickle.dump(config, f)
...
def generate_embeddings_BAAI(config):
    ...
    with open('../data/MOOCRadar/embedding_BAAI.pkl', 'wb') as f:
        pickle.dump(config, f)    
    with open('MOOCRadar/result/embedding/embedding_BAAI.pkl', 'wb') as f:
        pickle.dump(config, f)
...
def generate_embeddings_m3e(config):
    ...
    with open('../data/MOOCRadar/embedding_m3e.pkl', 'wb') as f:
        pickle.dump(config, f)
    with open('MOOCRadar/result/embedding/embedding_m3e.pkl', 'wb') as f:
        pickle.dump(config, f)
...
def generate_embeddings_Instructor(config):
    ...
    with open('../data/MOOCRadar/embedding_instructor.pkl', 'wb') as f:
        pickle.dump(config, f)
    with open('MOOCRadar/result/embedding/embedding_instructor.pkl', 'wb') as f:
        pickle.dump(config, f)
...
def run(arg):
    ...
    with open('MOOCRadar/result/data/map.pkl', 'rb') as f:
        config_map = pickle.load(f)
    TotalData = pd.read_csv("MOOCRadar/result/data/TotalData.csv", header=None, names=['stu', 'exer', 'answervalue'])
    ...
    config = generate_text(config, config_map, TotalData, question_info_map)

    # print(len(config["knowledge_text"]))
    # print(len(config["exercise_text"]))
    # print(len(config["student_text"]))
